...
public class BodyEditorLoader {
	...
	private RigidBodyModel readRigidBody(JsonValue bodyElem) {
		...
		for (JsonValue polygon : polygonsElem.iterator()) {
			PolygonModel polygonModel = new PolygonModel();
			rbModel.polygons.add(polygonModel);
			
			for (JsonValue vertex : polygon.iterator()) {
				float x = vertex.get("x").asFloat();
				float y = vertex.get("y").asFloat();
		...
		for (JsonValue circle : circlesElem) {
			CircleModel circleModel = new CircleModel();
			rbModel.circles.add(circleModel);
			
			circleModel.center.x = circle.get("cx").asFloat();
			circleModel.center.y = circle.get("cy").asFloat();
			circleModel.radius = circle.get("r").asFloat();
		}
